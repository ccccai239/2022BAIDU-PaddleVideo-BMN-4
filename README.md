...
我们对训练数据集进行初步分析，统计出动作长度的分布直方图，如下图1。对于乒乓球时许动作定位任务来说，比赛的数据集特征是以pkl格式保存，读取pkl之后以(num_of_frames, 2048)向量形式代表单个视频特征，其中num_of_frames是不固定的且数量大，不能直接运用于训练；并且从图可以看出乒乓球每个动作时间非常短，多数集中于0~1s。因此，我们认为合理有效的数据分割方式对本次任务非常重要。
...
### 整体优化思路
我们方案的整体架构主要分为数据处理、模型训练、模型导出与推理预测四部分；该方案整体框架是基于官方baseline，采用BMN为基本网络模型，主要从数据预处理与训练模型的学习率调整策略两方面进行优化改进。
* 数据预处理：我们尝试使训练集与测试集采用不同的数据分割方法，使两者的滑动窗口移动方式不同，对于测试集，更保证分割后动作的完整性。
* 学习率调整策略：我们尝试Decay、Warmup与余弦退火衰减等多种学习率调整方法。
* 预测后处理：由于数据分割后的每个切片不一定包含提案，因此对所有预测的提案结果进行汇总并使用非极大值抑制算法（soft-NMS）进一步筛选。
![]
...
```
    ...
    /home/aistudio/PaddleVideo/applications/TableTennis
    save feature for bmn ...
    miss number (broken sample): 111
...
```
    ...
    (Label) Deleted size: 111
    (Label) Fixed size: 19717
...
```
    ...
    /home/aistudio/work/data/test_npy
    15631
...
2. 基于每4秒一分的划分方法上，将滑动窗口的步长改为2秒，保证测试集动作的完整性，优化特征数据。其最终的测试的得分为46.96142，说明优化数据处理提升效果最大。
3. 采用非极大抑制算法合并测试结果
4. 采用多步长衰减与Warmup的余弦退火衰减等多种学习率调整方法，其A榜测试提交得分对比如下：
